<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ include file="/WEB-INF/views/camping/common.jsp"%>
<%@ include file="/WEB-INF/views/user/menuBar.jsp"%>

<div class="container">
	<!-- 배너이미지  -->
	<div style="margin-bottom:60px;">
		 <a href=""><img class="banner" src="/img/banner.jpg" style="width:1140px; height:400px;"></a>
	</div>
	<!-- <div style="text-align:right; margin-right:50px; margin-top:60px; "><a href="/list">전체보기>></a></div>
	<c:forEach items="${clist}" var="product" varStatus="i" begin="0" end="5">       
	      <div class="row" style="width:350px; margin:10px; float:left; "  >
	        <div class="col-sm-3">
	          <div class="card" style="border:1px solid grey; width:300px; height:410px;">
	          	<div style="width:300px;height:300px;">
		          <c:forEach items="${product.picture}" var="pic" begin="0" end="0">
		           <a href="/detail/${product.camp_id}"><img class="card-img-top img" data-src="${i.count}" src="/image/${pic.pic_name}" alt="Card image" style="width:298px; height:298px;"></a> 
		          </c:forEach>
	  			</div>
		        <div class="card-body">
			          <h4>${product.camp_title}</h4>
			          <p class="card-text">${product.address}</p>
		        </div>
	          </div>
	        </div>
	      </div>
	</c:forEach>-->
</div> 
<div style="clear:both;margin-bottom:60px;"></div>
<!--  좋아요가 많은 순(3개출력) -->
<div class="container" style="magin:0px;padding:0px;">
<hr style="width:200px; margin:auto;">
	<div style="text-align:right; margin-right:50px; margin-top:60px; "><a href="/likelist">전체보기>></a></div>
<h2 style="margin-left:50px;font-weight:bold;">Hot</h2>
<br>
<c:forEach items="${likeList}" var="product" begin="0" end="2">       
      <div class="row" style="width:350px; margin:10px; float:left; ">
        <div class="col-sm-3">
          <div class="card" style="border:1px solid grey; width:300px; height:410px;">
          	<div style="width:300px;height:300px;">
	          <c:forEach items="${product.picture}" var="pic" begin="0" end="0">
	           <a href="/detail/${product.camp_id}"><img class="card-img-top img" data-src="${i.count}" src="/image/${pic.pic_name}" alt="Card image" style="width:298px; height:298px;"></a> 
	          </c:forEach>
  			</div>
	        <div class="card-body">
		          <h4>${product.camp_title}</h4>
		          <p class="card-text">${product.address}</p>
	        </div>
          </div>
        </div>
      </div>
</c:forEach>
</div>
<div style="clear:both;margin-bottom:60px;"></div>

<!-- 새로 등록한 캠핑장(3개출력) -->
<div class="container"  style="magin:0px;padding:0px;">
<hr style="width:200px; margin:auto;">
	<div style="text-align:right; margin-right:50px; margin-top:60px; "><a href="/list">전체보기>></a></div>
<h2 style="margin-left:50px;font-weight:bold;">New</h2>
<br>
<c:forEach items="${campNew}" var="product" begin="0" end="2">       
      <div class="row" style="width:350px; margin:10px; float:left; ">
        <div class="col-sm-3">
          <div class="card" style="border:1px solid grey; width:300px; height:410px;">
          	<div style="width:300px;height:300px;">
	          <c:forEach items="${product.picture}" var="pic" begin="0" end="0">
	           <a href="/detail/${product.camp_id}"><img class="card-img-top img" data-src="${i.count}" src="/image/${pic.pic_name}" alt="Card image" style="width:298px; height:298px;"></a> 
	          </c:forEach>
  			</div>
	        <div class="card-body">
		          <h4>${product.camp_title}</h4>
		          <p class="card-text">${product.address}</p>
	        </div>
          </div>
        </div>
      </div>
</c:forEach>
</div>

<div style="clear:both;"></div>
<%@ include file="../includes/footer.jsp"%>